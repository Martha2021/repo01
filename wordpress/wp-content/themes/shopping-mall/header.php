...
$shopping_mall_option['header_style'] = ecommerce_star_header_class();
...
?>
<a class="skip-link screen-reader-text" href="#primary" ><?php esc_html_e( 'Skip to content', 'shopping-mall' ); ?></a>
<header id="masthead" class="<?php echo esc_attr(ecommerce_star_header_class()); ?> site-header" role="banner" >
	
	<?php get_template_part( 'template-parts/header/header', $shopping_mall_option['mini_header_style'] );	?>

	<div class="container">
		<?php
		if($shopping_mall_option['header_woocommerce'] && class_exists( 'WooCommerce' )){
		...
		//display slider
		if((is_front_page() || is_home()) && $shopping_mall_option['slider_in_home_page'] ){
			get_template_part( 'template-parts/slider', 'section' );	
		}
				
		// breadcrumb not displayed with transparent header
		if ($shopping_mall_option['enable_breadcrumb'] ) {
